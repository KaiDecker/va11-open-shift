...
每个新的场景请求会推进一段持久世界时间，并从最新权威事件选择参与者和当前顾客。世界从原版主线及结局事件发生之后继续运转；已经建立的人物关系和生活状态不会被重置。顾客先明确点单，Jill 使用独立玩家上下文确认，随后游戏切入原版配方书与调酒界面。模型每次只负责当前角色的一句：Agent 只能看到自己的角色核心、定性状态、相关长期记忆和此前公开台词；Jill 只能看到公开对白、固定角色核心和规则层确认的服务结果。金钱、目标数值和数据库时间不会进入对白观察。
...
阶段 6 已完成单次“Agent 点单—Jill 回应—原版调酒—规则判定—分支反应”闭环。阶段 7 会把它扩展为预生成的整日有限剧情图，减少等待并支持单日多位顾客；阶段 8 再接入营业结束、Jill 回家、原版存档槽与下一营业日循环。
