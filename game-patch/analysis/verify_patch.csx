...
if (Data.Code.ByName("gml_Object_obj_textbox_Create_0") is null) throw new Exception("Missing textbox Create entrypoint");
if (Data.Code.ByName("gml_Object_obj_textbox_Step_1") is null) throw new Exception("Missing textbox Step entrypoint");
...
string dialogCreate = Decompile("gml_Object_dialog_control_Create_0");
...
if (!controllerCreate.Contains("ag_request_scope") || !controllerStep.Contains("ag_request_scope") || !controllerHttp.Contains("ag_request_scope") || !mixControl.Contains("ag_request_scope")) throw new Exception("Dialogue request IDs were reused when a later day created a new controller");
if (!dialogCreate.Contains("global.cur_day >= 1001") || !dialogCreate.Contains("instance_create(x, y, ag_bridge_controller)")) throw new Exception("Later Open Shift days did not create the bar dialogue controller");
string dialogStep = Decompile("gml_Object_dialog_control_Step_0");
string textboxCreate = Decompile("gml_Object_obj_textbox_Create_0");
string textboxStep = Decompile("gml_Object_obj_textbox_Step_1");
string textboxLoadbox = Decompile("gml_Script_textbox_loadbox");
...
if (!controllerHttp.Contains("string_count(\"[\", ag_order_display_text) > 0") || !controllerHttp.Contains("string_count(\"]\", ag_order_display_text) > 0") || !controllerHttp.Contains("ag_ack_error_code") || !controllerHttp.Contains("string_count(\"[\", ag_ack_error_code")) throw new Exception("Order display command injection guard was missing");
...
if (!textboxCreate.Contains("ag_open_shift_wait = global.ag_memory_textbox_wait") || !textboxStep.Contains("ag_open_shift_wait") || !textboxStep.Contains("textbox_closing = 0") || textboxStep.Contains("variable_instance_exists")) throw new Exception("Provider placeholder textbox compatibility guard was missing");
if (controllerStep.Contains("ag_wait_box.textbox_skip_possible") || controllerStep.Contains("ag_error_box.textbox_skip_possible")) throw new Exception("Bridge directly modified vanilla textbox internals");
if (!textboxLoadbox.Contains("ag_memory_textbox_active") || !textboxLoadbox.Contains("global.ag_memory_textbox_lines") || !controllerStep.Contains("textbox_create_alt(\"\", 0, 1)")) throw new Exception("Memory-backed native textbox adapter was missing");
...
if (!mixControl.Contains("global.cur_day >= 1001") || !mixControl.Contains("/v1/orders/jobs")) throw new Exception("Later Open Shift days did not queue drink results");
...
if (!tabletCreate.Contains("global.cur_day >= 1001") || !tabletCreate.Contains("ag_tablet_controller")) throw new Exception("Later Open Shift days did not install the tablet controller");
...
if (!preloadCreate.Contains("/v1/story/prepare") || !preloadCreate.Contains("ag_preload_scope") || !preloadStep.Contains("ag_preload_scope") || !preloadStep.Contains("ag_prefetch_ready") || !preloadHttp.Contains("今日世界状态已准备完成") || !preloadHttp.Contains("global.jillcomment")) throw new Exception("Apartment story preparation gate or unique daily request scope was missing");
if (!preloadHttp.Contains("shift_phase") || !preloadHttp.Contains("global.ag_story_day") || !preloadStep.Contains("deadline = global.datestring") || !preloadStep.Contains("distraction = \"Glitch City") || !preloadStep.Contains("unlocked = \"今日世界状态已准备完成") || !preloadStep.Contains("dismiss = \"点击鼠标关闭\"") || !preloadStep.Contains("other.ag_preload_state == 1") || preloadStep.Contains("else if (ag_preload_state == 1)") || showRoomCreate.Contains("global.datestring = \"O.S. DAY 1\"") || !showRoomCreate.Contains("global.ag_story_day")) throw new Exception("O.S. apartment day synchronization was missing");
if (!preloadHttp.Contains("本地服务暂时不可用") || !preloadHttp.Contains("今日营业还没准备好")) throw new Exception("Local story fallback was missing");
if (!showRoomCreate.Contains("OPEN SHIFT") || !showRoomCreate.Contains("global.cur_day >= 1001") || !showRoomCreate.Contains("正在准备今天的营业") || !showRoomCreate.Contains("room_text") || !showRoomCreate.Contains("点击鼠标关闭") || !controllerHttp.Contains("instance_create(x, y, new_day)") || preloadStep.Contains("instance_create(x, y, obj_textbox)") || preloadStep.Contains("instance_create(x, y, out_of_apartment)")) throw new Exception("Original room popup introduction or manual bar entry was missing");
...
if (!toWorkMouse.Contains("global.cur_day >= 1001") || !toWorkMouse.Contains("ag_story_ready") || !toWorkMouse.Contains("global.ag_prefetch_day == global.ag_story_day") || !toWorkMouse.Contains("instance_create(x, y, out_of_apartment)")) throw new Exception("Later Open Shift days did not wait for story preparation before work");
